...
void disable_timer2(void) {
    // Disable Timer 2 Clock by clearing the TIM2EN bit in RCC_APB1ENR register
    RCC->APB1ENR &= ~RCC_APB1ENR_TIM2EN;
    
    // Optionally, reset the Timer 2 configuration if needed
    // By writing 0 to the TIM2 registers, you can clear its configuration
    TIM2->CR1 = 0;  // Reset Timer 2 Control Register 1
    TIM2->CR2 = 0;  // Reset Timer 2 Control Register 2
    TIM2->SMCR = 0; // Reset Timer 2 Slave Mode Control Register
    TIM2->DIER = 0; // Disable Timer 2 Interrupts
    TIM2->SR = 0;   // Clear Timer 2 Status Register
}